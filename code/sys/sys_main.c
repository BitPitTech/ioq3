...
qboolean Sys_exit_if_hosted() {
    int isDeveloper = Cvar_VariableIntegerValue("com_developer");

    if (!isDeveloper) {
        Com_Quit_f();

        SV_Shutdown("Please return to https://bitpit.io");
        Com_Quit_f();
        return qtrue;
    }
    return qfalse;
