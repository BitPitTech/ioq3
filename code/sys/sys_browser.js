var LibrarySys = {
	...
	Sys_FOpen: function (ospath, mode) {
		var stringMode = Pointer_stringify(mode);
		if (stringMode == "rb") {
			stringMode = "r";
		}
		try {
			return FS.open(Pointer_stringify(ospath), stringMode);
		} catch (e) {
			return 0;
		}
	},
	...
	Sys_BeginAudioTransaction: function() {
		if (AL.transactionStarted) {
			if (_IsDeveloper()) {
				throw new Error('Transaction already started, no reentrancy is allowed');
			}
		}
		AL.transactionStarted = true;
	},
	Sys_CommitAudioTransaction: function() {
		if (AL.transactionContext) {
			AL.updateListenerSpace(AL.transactionContext);
		}

		AL.transactionContext = null;

		if (!AL.transactionStarted) {
			throw new Error('Transaction hasn\'t started.');
		}
		AL.transactionStarted = false;
	},
	Sys_GLCreated: function () {
		var canvas = Module['canvas'];

		function prepareAudioTransactionOptimizations() {
			var originalUpdateListenerSpace = AL.updateListenerSpace;
			AL.updateListenerSpace = function(ctx) {
				if (AL.transactionStarted) {
					if (AL.transactionContext && AL.transactionContext !== ctx) {
						throw new Error('Attempt to peform transaction across different audio contexts, this is not allowed.');
					}

					AL.transactionContext = ctx;
					return;
				}
				
				originalUpdateListenerSpace(ctx);
			};
		};
		function registerFullScreenClickHandler() {
			var listener = function() {
				Object.getOwnPropertyNames(AL.contexts).forEach(function (name) {
					var context = AL.contexts[name];
					if (context.audioCtx.state == 'suspended') {
						context.audioCtx.resume();
						ioq3.canvas.removeEventListener('click', listener);
					}
				});
			};
			canvas.addEventListener('click', listener);
		};
		function registerResumeAudioSessionOnClick() {
			canvas.addEventListener('click', function() {
				// Module.requestFullscreen(true, true);

				// requestFullscreen(true, false);
		...
		registerFullScreenClickHandler();
		registerResumeAudioSessionOnClick();
		prepareAudioTransactionOptimizations();
	}
};
